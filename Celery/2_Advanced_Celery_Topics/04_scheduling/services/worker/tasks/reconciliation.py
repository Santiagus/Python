...
def _calculate_reconciliation_hash(payload_str: str) -> str:
    """Compute SHA-256 cryptographic verification checksum for reconciliation data.

    Args:
        payload_str: Colon-delimited reconciliation state string.
...
async def _execute_reconciliation(
    ...
    async with session_factory() as session:
        try:
            ...
            hash_payload = (
                f"{period_date_str}:{total_credits}:{total_debits}:{net_movement}:{report_status}:{discrepancy}"
            )
            verification_hash = _calculate_reconciliation_hash(hash_payload)

            # 5. Persist new report or perform in-place verification update
            now_utc = datetime.now(timezone.utc)
            if existing_report is not None:
                prev_status = existing_report.status
                logger.info(
                ...
                existing_metadata = dict(existing_report.metadata_json or {})
                existing_metadata.update(
                    {
                        "entry_count": entry_count,
                        "re-verified": prev_status in ("balanced", "discrepancy_detected"),
                    }
                )
                existing_report.metadata_json = existing_metadata
                report_id = existing_report.report_id
                outcome = "verified_existing" if prev_status in ("balanced", "discrepancy_detected") else "completed"
            else:
                logger.info(
            ...
            logger.exception("Reconciliation failed for period %s: %s", period_date_str, exc)
            await session.rollback()
            try:
                fail_stmt = select(ReconciliationReport).where(ReconciliationReport.period_date == period)
                fail_res = await session.execute(fail_stmt)
                fail_report = fail_res.scalar_one_or_none()
                now_utc = datetime.now(timezone.utc)
                if fail_report is not None:
                    fail_report.status = "failed"
                    fail_meta = dict(fail_report.metadata_json or {})
                    fail_meta.update(
                        {
                            "error": str(exc),
                    ...
                    fail_report.metadata_json = fail_meta
                else:
                    fail_report = ReconciliationReport(
                        report_id=uuid.uuid4(),
                        period_date=period,
                        status="failed",
                ...
                logger.error(
                    "Failed to persist failed status for period %s: %s",
                    period_date_str,
                    inner_exc,
                )
                await session.rollback()

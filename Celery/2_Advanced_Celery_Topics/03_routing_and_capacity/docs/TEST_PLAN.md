...
| **TC-17: Process-Local Account In-Memory Cache** | `test_api.py::test_account_cache_hit_and_miss`, `test_api.py::test_clear_account_cache` | Seeded account UUID, sequential instant payment requests, cache reset hook | 1. Initial request misses cache and populates `_ACCOUNT_CACHE` dictionary with 300s TTL.<br/>2. Subsequent requests resolve account existence in nanoseconds with zero SQL round-trips.<br/>3. `clear_account_cache()` cleanly evicts entries, maintaining test suite isolation. | Sub-microsecond memory validation, zero cache pollution |
| **TC-18: Zero-Refresh Ingestion Optimization** | `test_api.py::test_create_payment_instant_success` | Valid `POST /payments/instant` payload | 1. Application layer pre-generates UUID primary key and UTC timestamp in memory.<br/>2. Model commits directly to PostgreSQL without calling `await session.refresh()`.<br/>3. Exactly 1 SQL query (`INSERT`) is executed, eliminating redundant `SELECT`. | Ingestion latency slashed ($< 15\text{ ms}$), zero redundant SQL reads |
| **TC-19: PgBouncer Statement Cache Configuration** | `test_api.py::test_pgbouncer_statement_cache_zero` | SQLAlchemy engine initialization in `app/db.py` | 1. Inspect engine connect arguments.<br/>2. Assert `connect_args["statement_cache_size"] == 0` for asyncpg.<br/>3. Guarantees zero prepared statement collision when transactions are multiplexed across pooled server sockets. | Clean connection multiplexing, zero statement crash |
...
### Phase 5: Hardware Frontier Saturation, PgBouncer Multiplexing & Durability Validation
1. Deploy `pgbouncer` container (`edoburu/pgbouncer:latest`) in transaction pooling mode (`DEFAULT_POOL_SIZE=25`, `POOL_MODE=transaction`).
2. Configure asyncpg with `statement_cache_size=0` in `app/db.py`.
3. Implement in-memory L1 cache (`_ACCOUNT_CACHE`) and zero-refresh UUID response generation in `app/routes.py`.
4. Deduplicate indexes in `init.sql` and add partial indexes for active statuses (`WHERE status IN ('pending', 'processing')`).
5. Run Two-Stage Hardware Frontier Bisection Benchmark (`scripts/benchmark_bisection_capacity.py --hardware-frontier`), scaling to 8 instant containers and sustaining **$300.0\text{ req/s}$** ($P_{99} \le 83.0\text{ ms}$) under strict **`synchronous_commit = on`**.
6. Verify 100% statement & branch coverage: **125 passed of 125 tests** on 996 statements with 0 missed lines.

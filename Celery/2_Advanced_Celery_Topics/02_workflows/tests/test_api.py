...
class TestApplicationAPI:
    ...
    async def test_create_application(self, app) -> None:
    ...
    async def test_submit_dossier_and_poll_status(self, app, clean_dossier_manifest: dict[str, str]) -> None:
        ...
        async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as ac:
            ...
            status_resp = await ac.get(f"/api/v1/applications/{app_id}")
            assert status_resp.status_code == 200
            status_data = status_resp.json()
            assert status_data["application_id"] == app_id

            # 4. Query timing telemetry
            timing_resp = await ac.get(f"/api/v1/applications/{app_id}/timing")
            assert timing_resp.status_code in (200, 404)  # 404 if still in flight or 200 if completed

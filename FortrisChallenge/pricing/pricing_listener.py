...
async def message_unpack(redis, config):
    ...
    logging.info(
        f"There are {msg_pending} msg pending in {config['requests']['stream']} stream."
    )
...
async def process_message(redis, config):
    try:
        while True:
            message, message_id, timestamp, get_latest_data = await message_unpack(
                redis, config
            )
            if message:
                ...
                logging.info(
                    f"Deleted {config['responses']['stream']} {message_id} : {res}"
                )
                msg_pending = await redis.xlen(config["requests"]["stream"])
            else:
...
async def main():
    ...
    try:
        ...
        retval = await create_redis_group(
            redis,
            config["requests"]["stream"],
            config["requests"]["group"]["group_name"],
        )
